...
{   
     /**
     * @Route("/all", name="places_index" , methods={"GET"})
     */
    public function places(PlaceRepository $placeRepo): Response
    {
        return $this->render('home/places/places.html.twig', [
            "places"=>$placeRepo->findAll(),
        ]);
    }
    /**
     * @Route("/place/{id}", name="place_go")
     */
    public function go($id , PlaceRepository $placerepo): Response
    {
       $place=$placerepo->find($id);
       $trips=$place->getTrips();
       $size=$trips->count();
        return $this->renderForm('home/places/go.html.twig', [
            'trips' => $trips,"size"=>$size ,"name"=>$place->getTitre(),
